...
from Video_Crawler import youtube_video_download
...
class PlayListDetails:
	...
	def DownloadPlaylistVideos(self,number_of_videos=None):
		self.PlaylistAllVideos()
		if number_of_videos==None:
			print(self.video_links)
		else:
			if isinstance(number_of_videos,int):
				if number_of_videos == 0:
					print("Invalid Number!")
				elif number_of_videos>len(self.video_links):
					print(f"In this playlist only {len(self.video_links)} videos are there!") 
				else:
					print(self.video_links[0:number_of_videos])
			else:
				print("Enter Number Only!")


y = PlayListDetails('https://www.youtube.com/playlist?list=PLsyeobzWxl7r2ukVgTqIQcl-1T0C2mzau')
y=y.DownloadPlaylistVideos()